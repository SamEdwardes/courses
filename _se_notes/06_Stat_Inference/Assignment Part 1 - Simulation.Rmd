---
title: 'Assignment Part 1: Simulation Exercise'
...
df <- data.frame(simulated.mean = sim_means,
                 simulated.var = sim_var)

# preview the dataframe
head(df);tail(df)
```

The charts below show the output of 4 random exponential distribution simulations.
```{r}
# generate 4 expondential distributions to show as examples
set.seed(5678)
sample1 <- rexp(n_obs, lambda); sample2 <- rexp(n_obs, lambda); sample3 <- rexp(n_obs, lambda); sample4 <- rexp(n_obs, lambda)

# combine the distributions into a dataframe for plotting
sample_df <- data.frame(num = c(sample1, sample2, sample3, sample4),
                        sample = c(rep('sample1',n_obs), rep('sample2',n_obs), rep('sample3',n_obs), rep('sample4', n_obs)))

# plot the 4 samples
g <- ggplot(sample_df, aes(y = num, group = sample)) + 
    facet_grid(.~sample) +
    geom_boxplot(aes(x = ''), outlier.shape = NA, width = 0.3) + 
    stat_summary(aes(x = '', ymax = ..y.., ymin = ..y..),fun.y = mean, color='blue', geom="errorbar", linetype = "dashed") + # plot the actual means
    geom_point(aes(x = ''),alpha = 0.2, size = 3) + # plot all the points
    geom_hline(yintercept = u, color = 'red', alpha = 0.5) + # plot the theoretical mean
    labs(title = "Sample of exponential distribution simulations", x = 'samples')
g
```

As can be seen in the 4 random samples above, the mean of the each sample differs greatly compared to that of the theoretical mean: `r u`. The theoretical mean is visualized using the solid red line. The mean for each sample is visualized using the dotted blue line.
...
g1 <- g1 + labs(title = "Distribution of simulated means", x = "simulated mean")
...
g2 <- g2 + labs(title = "Distribution of simulated variances", x = "simulated variance")
